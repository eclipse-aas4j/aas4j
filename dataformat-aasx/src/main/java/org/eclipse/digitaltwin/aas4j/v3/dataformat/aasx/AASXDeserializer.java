...
/**
 * The AASX package converter converts a aasx package into a list of aas, a list of submodels a list
 * of assets, a list of Concept descriptions
 */
public class AASXDeserializer {
  ...
  private static final String AASPEC_RELTYPE_BACKWARDSCOMPATIBLE =
   ...
   * @throws IOException if creating input streams for aasx fails
   */
  public AASXDeserializer(XmlDeserializer xmlDeserializer, InputStream inputStream)
  ...
  public AASXDeserializer(
      XmlDeserializer xmlDeserializer, JsonDeserializer jsonDeserializer, InputStream inputStream)
      throws InvalidFormatException, IOException {
    aasxRoot = OPCPackage.open(inputStream);
    this.xmlDeserializer = xmlDeserializer;
  ...
  protected MetamodelContentType getContentType() throws InvalidFormatException, IOException {
    ...
    // We also check for the none official content types "test/xml" and "text/json", which are
    // commonly used
    switch (packagePart.getContentType()) {
      ...
      default:
        throw new RuntimeException(
            "The following content type is not supported: " + packagePart.getContentType());
    }
    return contentType;
  }
   ...
   *
   * @deprecated This method will be replaced by the method {@link
   *     AASXDeserializer#getResourceString()}.
   * @throws InvalidFormatException if aasx package format is invalid
   * @throws IOException if creating input streams for aasx fails
   */
  @Deprecated
  ...
  public List<InMemoryFile> getRelatedFiles()
    ...
    List<String> filePaths =
        parseReferencedFilePathsFromAASX().stream()
            .filter(AASXUtils::isFilePath)
  ...
  private PackagePart getPackagePart(OPCPackage aasxPackage)
      throws InvalidFormatException, IOException {
  ...
  private String getResourceString(OPCPackage aasxPackage)
      throws InvalidFormatException, IOException {
  ...
  private PackageRelationshipCollection getXMLDocumentRelation(PackagePart originPart)
  ...
  private String getXMLPart(PackagePart originPart) throws InvalidFormatException {
    if (isCompatibilityModeNeeded(originPart)) {
      logger.info(
          "AASX contains wrong Relationship namespace. This may be related to the AASX being created with an old version of AASX Package Explorer or an old version of AAS4J. Future compatibility with the wrong namespace may not be guaranteed");
  ...
  private boolean isCompatibilityModeNeeded(PackagePart originPart) throws InvalidFormatException {
    PackageRelationshipCollection originRelationshipsBackwardsCompatible =
        originPart.getRelationshipsByType(AASPEC_RELTYPE_BACKWARDSCOMPATIBLE);

    return originRelationshipsBackwardsCompatible.size() > 0;
  }

  private void checkNumberOfRelationsForValidity(PackageRelationshipCollection originRelationships)
      throws InvalidFormatException {
  ...
  /**
   * Load the referenced filepaths in the submodels such as PDF, PNG files from the package
   *
  ...
  private InMemoryFile readFile(OPCPackage aasxRoot, String filePath)
      throws InvalidFormatException, IOException {
    PackagePart part =
        aasxRoot.getPart(
            PackagingURIHelper.createPartName(AASXUtils.removeFilePartOfURI(filePath)));
