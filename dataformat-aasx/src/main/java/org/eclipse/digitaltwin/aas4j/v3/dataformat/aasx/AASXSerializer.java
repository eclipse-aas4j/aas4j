...
/**
 * This class can be used to generate an .aasx file from Metamodel Objects and the Files referred to
 * in the Submodels
 */
public class AASXSerializer {
  ...
  public static final String OPC_NAMESPACE =
  ...
  /**
   * Generates the .aasx file and writes it to the given OutputStream, by using XML as the default
   * content type.
   *
   * @param environment the aas environment that will be included in the aasx package as an xml
   *     serialization
    ...
    environment.getAssetAdministrationShells().stream()
        ...
        .forEach(
            aas ->
                createParts(
                    files,
                    AASXUtils.removeFilePartOfURI(
                        aas.getAssetInformation().getDefaultThumbnail().getPath()),
                    rootPackage,
                    rootPackage,
                    aas.getAssetInformation().getDefaultThumbnail().getContentType(),
                    AAS_THUMBNAIL_RELTYPE));
   ...
   */
  private void storeFilesInAASX(
      Environment environment,
      Collection<InMemoryFile> files,
      OPCPackage rootPackage,
      PackagePart xmlPart) {
    findFileElements(environment)
        .forEach(
            file ->
                createParts(
                    files,
                    AASXUtils.removeFilePartOfURI(file.getValue()),
                    rootPackage,
                    xmlPart,
                    file.getContentType(),
                    AASSUPPL_RELTYPE));
  ...
  private void createParts(
    ...
    try {
      ...
      if (exists && !Objects.equals(contentType, part.getContentType())) {
        logger.info(
            "File '{}' already exists with different content type (expected: {}, actual: {})",
            filePath,
            part.getContentType(),
            contentType);
      }
    ...
    if (!exists) {
      try {
        ...
        createAASXPart(
            rootPackage, xmlPart, filePath, contentType, relType, content.getFileContent());
      } catch (RuntimeException e) {
        // Log that a file is missing and continue building the .aasx
        logger.warn(
            "Could not add File '{}'. It was not contained in given InMemoryFiles.", filePath, e);
      }
    }
  }
  ...
  /**
   * Generates a UUID. Every element of the .aasx needs a unique Id according to the specification
   *
   * @return UUID
   */
   ...
   */
  private PackagePart createAASXPart(
      OPCPackage root,
      RelationshipSource relateTo,
      String path,
      String mimeType,
      String relType,
      byte[] content) {
    if (mimeType == null || mimeType.equals("")) {
      throw new RuntimeException(
          "Could not create AASX Part '" + path + "'. No MIME_TYPE specified.");
    }
  ...
  private void writeDataToPart(PackagePart part, byte[] content) {
    try (OutputStream ostream = part.getOutputStream()) {
      ...
      throw new RuntimeException(
          "Failed to write content to AASX Part '" + part.getPartName().getName() + "'", e);
    }
  }

  /**
   * Gets the File elements from an environment searches in SubmodelElementCollections
   *
   * @param environment the Environment
