...
public class JsonDeserializerTest {
  ...
  public void testReadCustomDataSpecification() throws DeserializationException {
    ...
    deserializer.useImplementation(
        DataSpecificationContent.class, DefaultDummyDataSpecification.class);
    Environment env =
        deserializer.read(Examples.ENVIRONMENT_CUSTOM_DATA.fileContentStream(), Environment.class);
  ...
  public void testReadConceptDescriptionWithPhysicalUnit()
      throws IOException, DeserializationException {
    ConceptDescription expected =
        Examples.CONCEPT_DESCRIPTION_DATA_SPECIFICATION_PHYSICAL_UNIT.getModel();
    ConceptDescription actual =
        deserializerToTest.read(
  ...
  public void testReadEmptyReferableList() throws DeserializationException {
    ...
    deserialized =
        deserializerToTest.readList(JsonNodeFactory.instance.arrayNode(), Referable.class);
    assertEquals(emptyList, deserialized);
  ...
  private Object readFromString(ExampleData<?> exampleData)
      throws IOException, DeserializationException {
    Object model = exampleData.getModel();
    if (model instanceof Collection<?>) {
      Collection<?> coll = (Collection<?>) model;
      return deserializerToTest.readList(
          exampleData.fileContent(), coll.iterator().next().getClass());
    }
  ...
  private Object readFromStream(ExampleData<?> exampleData) throws DeserializationException {
    ...
    if (model instanceof Collection<?>) {
      Collection<?> coll = (Collection<?>) model;
      return deserializerToTest.readList(
          exampleData.fileContentStream(), coll.iterator().next().getClass());
    }
    return deserializerToTest.read(exampleData.fileContentStream(), model.getClass());
  }

  private Object readFromNode(ExampleData<?> exampleData)
      throws IOException, DeserializationException {
    Object model = exampleData.getModel();
    if (model instanceof Collection<?>) {
      Collection<?> coll = (Collection<?>) model;
      return deserializerToTest.readList(
          exampleData.getJsonNode(), coll.iterator().next().getClass());
    }
    return deserializerToTest.read(exampleData.getJsonNode(), model.getClass());
  }

  private void checkImplementationClasses(
      Environment environment,
      Class<? extends Submodel> submodelImpl,
      Class<? extends Property> propertyImpl) {
    environment
        .getSubmodels()
        .forEach(
            submodel -> {
