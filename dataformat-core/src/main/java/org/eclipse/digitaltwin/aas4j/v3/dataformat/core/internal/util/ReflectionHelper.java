...
/**
 * Helper class to collect relevant data needed for ReflectionAnnotationIntrospector via reflection.
 */
public class ReflectionHelper {
  ...
  /** Name of package where the generated default implementation files are defined */
  public static final String DEFAULT_IMPLEMENTATION_PACKAGE_NAME = MODEL_PACKAGE_NAME + ".impl";

  /**
   * Name of package where the json mixins are defined. These mixins are automatically added to
   * JsonSerializer and JsonDeserializer.
   */
  public static final String JSON_MIXINS_PACKAGE_NAME =
      ROOT_PACKAGE_NAME + ".dataformat.json.internal.mixins";

  /**
   * Name of package where the xml mixins are defined. These mixins are automatically added to
   * XmlSerializer and XmlDeserializer.
   */
  public static final String XML_MIXINS_PACKAGE_NAME =
      ROOT_PACKAGE_NAME + ".dataformat.xml.internal.mixins";
  ...
  /**
   * Distinct root superclasses of which classify a class to include type information via the
   * modelType property
   */
  public static final Set<Class<?>> MODEL_TYPE_SUPERCLASSES =
      Set.of(Referable.class, DataSpecificationContent.class);

  /** Expanded list of all classes that shall be annotated with the modelType property. */
  public static final Set<Class<?>> TYPES_WITH_MODEL_TYPE;

  /** Map of all interfaces and their subinterfaces defined in the MODEL_PACKAGE_NAME package. */
  ...
  /**
   * Expanded list of all mixin classes defined in the JSON_MIXINS_PACKAGE_NAME package together
   * with the corresponding class they should be applied to.
   */
  public static final Map<Class<?>, Class<?>> JSON_MIXINS;

  /**
   * Expanded list of all mixin classes defined in the XML_MIXINS_PACKAGE_NAME package together with
   * the corresponding class they should be applied to.
   */
  public static final Map<Class<?>, Class<?>> XML_MIXINS;

  /**
   * Expanded list of all default implementations in the DEFAULT_IMPLEMENTATION_PACKAGE_NAME package
   * together with the interface from the MODEL_PACKAGE_NAME package they are implementing.
   */
  @SuppressWarnings("rawtypes")
  public static final List<ImplementationInfo> DEFAULT_IMPLEMENTATIONS;

  /**
   * List of interfaces from the MODEL_PACKAGE_NAME package that are known to not have any default
   * implementation and therefore are excluded explicitely.
   */
  ...
  /**
   * Returns whether the given class is an interface and from within the MODEL_PACKAGE_NAME package
   *
   * @param type the class to check
   * @return whether the given class is an interface and from within the MODEL_PACKAGE_NAME package
   */
  ...
  public static boolean isDefaultImplementation(Class<?> type) {
    return DEFAULT_IMPLEMENTATIONS.stream()
        .anyMatch(x -> Objects.equals(x.getImplementationType(), type));
  }
  ...
  public static boolean hasDefaultImplementation(Class<?> interfaceType) {
    return DEFAULT_IMPLEMENTATIONS.stream()
        .anyMatch(x -> x.getInterfaceType().equals(interfaceType));
  }

  /**
   * Returns the default implementation for an aas interface or null if the class is no aas
   * interface or does not have default implementation
   *
   * @param interfaceType the interface to check
   * @param <T> the implementing class
   * @return the default implementation type for given interfaceType or null if the class is no aas
   *     interface or does not have default implementation
   */
  ...
  /**
   * Returns whether the given class is an interface from within the MODEL_PACKAGE_NAME package as
   * well as a default implementation or not
   *
   * @param type the class to check
   * @return whether the given class is an interface from within the MODEL_PACKAGE_NAME package as
   *     well as a default implementation or not
   */
  ...
  public static Class<?> getAasInterface(Class<?> type) {
    ...
    logger.debug(
        "class '{}' implements more than one AAS interface, but only most specific one is returned",
        type.getName());
    return implementedAasInterfaces.stream()
  ...
  public static Set<Class<?>> getAasInterfaces(Class<?> type) {
    ...
    if (type != null) {
      ...
      result.addAll(
          ClassUtils.getAllInterfaces(type).stream()
              .filter(x -> INTERFACES.contains(x))
  ...
  /**
   * Returns the AAS type information used for de-/serialization for a given class or null if type
   * information should not be included
   *
   * @param clazz the class to find the type information for
   * @return the type information for the given class or null if there is no type information or
  ...
  /**
   * Returns the most specific supertype that contains some AAS type information or null if there is
   * none
   *
   * @param clazz the class to find the type for
   * @return the most specific supertype of given class that contains some AAS type information or
  ...
  static {
    ScanResult modelScan =
        new ClassGraph().enableClassInfo().acceptPackagesNonRecursive(MODEL_PACKAGE_NAME).scan();
    ...
    INTERFACES_WITHOUT_DEFAULT_IMPLEMENTATION =
        getInterfacesWithoutDefaultImplementation(modelScan);
  }
  ...
  private static List<ImplementationInfo> scanDefaultImplementations(ScanResult modelScan) {
    ScanResult defaulImplementationScan =
        new ClassGraph()
            .enableClassInfo()
        ...
        .forEach(
            x -> {
              String interfaceName =
                  x.getSimpleName()
                      .substring(DEFAULT_IMPLEMENTATION_PREFIX.length()); // using conventions
              ClassInfoList interfaceClassInfos =
                  modelScan
                      .getAllClasses()
                      .filter(
                          y -> y.isInterface() && Objects.equals(y.getSimpleName(), interfaceName));
              if (interfaceClassInfos.isEmpty()) {
                logger.warn(
                    "could not find interface realized by default implementation class '{}'",
                    x.getSimpleName());
              } else {
                ...
                logger.debug(
                    "using default implementation class '{}' for interface '{}'",
                    x.getSimpleName(),
  ...
  private static Map<Class<?>, Class<?>> scanMixins(ScanResult modelScan, String packageName) {
    ScanResult mixinScan =
        new ClassGraph().enableClassInfo().acceptPackagesNonRecursive(packageName).scan();
  ...
  private static Set<Class<?>> scanModelTypes(ScanResult modelScan) {
    ...
    typesWithModelTypes =
        MODEL_TYPE_SUPERCLASSES.stream()
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      try {
        if (field.getType().isAssignableFrom(List.class)
            ...
            && ((List<?>) field.get(element)).isEmpty()) {
  ...
  private static Runnable createResetRunnable(Object element, Field field)
