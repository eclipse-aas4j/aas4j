...
public class MimeTypeMapper extends I4AASMapper<String, UAVariable> {
	...
	protected UAVariable createTargetObject() {
		...
		org.opcfoundation.ua._2011._03.uanodeset.UAVariable.Builder<Void> idVarBuilder = UAVariable.builder()
				.withValue().withAny(idStringValue).end().withDisplayName(createLocalizedText(MIME_TYPE_BROWSENAME))
				.withDataType(I4AASIdentifier.AASMimeDataType.getName()).withNodeId(ctx.newModelNodeIdAsString())
				.withBrowseName(createI4AASBrowseName(MIME_TYPE_BROWSENAME)).withAccessLevel(3L);
