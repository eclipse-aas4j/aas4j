...
public class FileParser extends ReferableParser<File> {
	...
	public FileParser(UANodeWrapper src, ParserContext ctx) {
		super(src, ctx);
	}

	@Override
	protected File createTargetObject() {
	...
	protected void parseAndAttachChildren() {
		...
		source.getI4AASProperty(MIME_TYPE_BROWSENAME).ifPresent(p -> {
			target.setMimeType(ParserUtils.extractValueAsString(p.getNodeVariable()));
		});
